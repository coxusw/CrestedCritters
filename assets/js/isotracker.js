(function () {
  ...
  const SERVICE_WORKER_PATH = "/assets/js/sw.js";
  ...
  const PRICE_PRESETS = ["online", "expo", "wholesale"];
  ...
  const DEFAULT_STATE = {
    ...
    pricePresets: {
      online: makeEmptyPreset(),
      expo: makeEmptyPreset(),
      wholesale: makeEmptyPreset()
    },
  ...
  function migrateState() {
    ...
    if (!state.pricePresets) {
      state.pricePresets = {
        online: {
          ...
          priceSections: state.priceSections || makeEmptyPreset().priceSections,
    ...
    PRICE_PRESETS.forEach((name) => {
      if (!state.pricePresets[name]) state.pricePresets[name] = makeEmptyPreset();
      const preset = state.pricePresets[name];
      preset.priceData = preset.priceData || {};
      preset.botanicalPriceData = preset.botanicalPriceData || {};
      preset.priceSections = Array.isArray(preset.priceSections) ? preset.priceSections : makeEmptyPreset().priceSections;
    ...
    state.activePricePreset = PRICE_PRESETS.includes(state.activePricePreset) ? state.activePricePreset : "online";
    ...
    state.salePrep.packagedItems = Array.isArray(state.salePrep.packagedItems) ? state.salePrep.packagedItems : [];
  ...
  function uniqueSources() {
    return [...new Set(
      state.colonies
        .flatMap(c => (c.sources || []).map(s => (s.name || "").trim()))
        .filter(Boolean)
    )].sort((a, b) => a.localeCompare(b));
  }

  function getAvailableTags() {
    return [...new Set([...BUILTIN_TAGS, ...(state.settings.customTags || [])])]
      .filter(Boolean)
      .sort((a, b) => a.localeCompare(b));
  }

  function getPreset(name) {
    return state.pricePresets[name];
  }
  ...
  function renderColonies() {
    ...
    let html = `
      <div class="iso-section-head">
        ...
        ${help("This is your main working list. Colony status is based on the most overdue task using your defaults or per-type thresholds.")}
      </div>
      <p class="iso-subtext">Oldest attention needs rise to the top so you can work the list efficiently.</p>
      ...
      <div class="iso-form-grid" style="margin-bottom:14px;">
        <div>
          <label>Search ${help("Searches colony name, type name, source names, source quantities, and source dates.")}</label>
        ...
        <div>
          <label>Tag ${help("Built-in tags are Breeding, For Sale, and Experimental. Add custom tags in Settings.")}</label>
        ...
        <div>
          <label>Source ${help("Filter colonies by a specific source/vendor name.")}</label>
  ...
  function showAddColonyForm() {
    ...
    app(`
      <div class="iso-section-head">
        ...
        ${help("Colony names must be unique. Type names can repeat when you keep multiple colonies of the same type.")}
      </div>
      ...
      <div class="iso-form-grid">
        ...
        <div>
          <label>Type Name ${help("Type name is used for combined population and per-type threshold settings.")}</label>
        ...
        <div>
          <label>Population / Units ${help("This is the tracked inventory amount for the colony. Use unit mode if you track cultures or other prepared units.")}</label>
      ...
      <div class="iso-section-head">
        ...
        ${help("Count mode subtracts sale unit size from colony quantity. Unit mode subtracts one unit for each prepared item.")}
      </div>
      ...
      <div class="iso-form-grid">
        ...
        <div>
          <label>Inventory Mode ${help("Count mode is for 10 count, 6 count, 50 count, etc. Unit mode is for 8 oz culture, cups, or other prepared units.")}</label>
      ...
      <div class="iso-form-grid">
        <div>
          <label>Packaging Supply Used ${help("Optional. If chosen, sale prep subtracts one supply item per packaged unit.")}</label>
  ...
  function openColony(index) {
    ...
    app(`
      ...
      <div class="iso-section-head">
        <h3 class="iso-card-title" style="margin:0;">For Sale Prep Setup</h3>
        ${help("Enable sale prep for this colony if you want it to appear in the For Sale Prep tab.")}
      </div>
      ...
      <label>Custom Note ${help("Any note changes and deletions are written to history with a timestamp.")}</label>
  ...
  async function saveColonyEdits(index) {
    ...
    const newPopulation = Math.max(0, toInt($("#editPopulation").value, 0));
    const oldTags = JSON.stringify(c.tags || []);
    const oldPrepEnabled = c.prepEnabled;
    const oldInventoryMode = c.inventoryMode;
    const oldSaleUnitLabel = c.saleUnitLabel;
    const oldSaleUnitSize = c.saleUnitSize;
    const oldSupply = c.containerSupplyId;
    ...
    if (oldPopulation !== newPopulation) {
      addHistory(c, "Population changed", `Population changed from ${oldPopulation} to ${newPopulation}.`);
    }

    if (oldNote !== newNote) {
      if (!oldNote && newNote) addHistory(c, "Added note", newNote);
      else if (oldNote && !newNote) addHistory(c, "Removed note", oldNote);
      else addHistory(c, "Edited note", `From "${oldNote}" to "${newNote}".`);
    }
    ...
    if (oldPrepEnabled !== c.prepEnabled) {
      addHistory(c, "Changed sale prep visibility", c.prepEnabled ? "Enabled in For Sale Prep." : "Disabled in For Sale Prep.");
    }

    if (oldInventoryMode !== c.inventoryMode) {
      addHistory(c, "Changed inventory mode", `Now using ${c.inventoryMode === "unit" ? "unit-based" : "count-based"} prep.`);
    }

    if (oldSaleUnitLabel !== c.saleUnitLabel || oldSaleUnitSize !== c.saleUnitSize) {
      addHistory(c, "Updated sale unit", `${c.saleUnitLabel}${c.inventoryMode === "count" ? ` (size ${c.saleUnitSize})` : ""}.`);
    }

    if (oldSupply !== c.containerSupplyId) {
      addHistory(c, "Changed packaging supply", "Updated linked packaging supply.");
    }
  ...
  function openSplitModal(index) {
    ...
    openModal(
      "Split Colony",
      `
        <p class="iso-subtext">Create a new colony from <strong>${esc(c.colonyName)}</strong>.</p>

        <div class="iso-kv-grid" style="margin-bottom:14px;">
          <div class="iso-stat">
            <div class="iso-stat-label">Original Colony</div>
            <div style="margin-top:4px;font-weight:900;">${esc(c.colonyName)}</div>
          </div>
          <div class="iso-stat">
            <div class="iso-stat-label">Type</div>
            <div style="margin-top:4px;font-weight:900;">${esc(c.typeName)}</div>
          </div>
          <div class="iso-stat">
        ...
        <div class="iso-note" style="margin-top:8px;">The new colony copies type, category, tags, note, image, care dates, sale prep settings, and sources. Date added becomes today.</div>
  ...
  async function splitColony(index) {
    ...
    const newColony = ensureColonyShape({
      ...
      sources: clone(original.sources || [])
    });

    original.population = originalPopulation - moveAmount;
    addHistory(original, "Split colony", `Moved ${moveAmount} to new colony "${newColonyName}".`);
  ...
  async function deleteColony(index) {
    ...
    if (!typeStillExists) {
      PRICE_PRESETS.forEach(name => {
        delete state.pricePresets[name].priceData[typeName];
        state.pricePresets[name].itemOrders.colonyTypes = state.pricePresets[name].itemOrders.colonyTypes.filter(x => x !== typeName);
      });
    }
  ...
  function renderPopulation() {
    ...
    let html = `
      <div class="iso-section-head">
        ...
        ${help("This combines total tracked quantity from all colonies of the same type.")}
      </div>
  ...
  function renderBotanicals() {
    let html = `
      <div class="iso-section-head">
        ...
        ${help("This tracks stock and notes only. Botanical prices are handled in the Price Sheet tab.")}
      </div>
  ...
  async function saveNewBotanical() {
    ...
    PRICE_PRESETS.forEach(name => {
      if (!state.pricePresets[name].botanicalPriceData[itemName]) {
        state.pricePresets[name].botanicalPriceData[itemName] = {
          included: true,
          section: "Botanicals",
          price: "",
          priceNote: ""
  ...
  async function deleteBotanical(index) {
    ...
    PRICE_PRESETS.forEach(name => {
      delete state.pricePresets[name].botanicalPriceData[itemName];
      state.pricePresets[name].itemOrders.botanicals = state.pricePresets[name].itemOrders.botanicals.filter(x => x !== itemName);
    });
  ...
  function renderSalePrep() {
    const eligible = state.colonies.filter(c => c.prepEnabled).sort((a, b) => a.colonyName.localeCompare(b.colonyName));
    const packagedByType = {};
    state.salePrep.packagedItems.forEach(item => {
      const key = item.typeName;
      if (!packagedByType[key]) packagedByType[key] = [];
      packagedByType[key].push(item);
    });

    let html = `
      <div class="iso-section-head">
        <h2 class="iso-section-title">For Sale Prep</h2>
        ${help("Prep moves quantity from a colony into packaged inventory. Count mode subtracts sale unit size. Unit mode subtracts one unit per prepared item.")}
      </div>
    ...
    html += `
      ...
      <div class="iso-section-head">
        ...
        ${help("Only colonies with sale prep enabled appear here. Packaging supply usage is subtracted automatically if you assigned one to that colony.")}
      </div>
    `;
        ...
        html += `
          <div class="iso-card">
            ...
            <div class="iso-meta">
              ...
              <div><strong>Unit Size:</strong> ${colony.inventoryMode === "count" ? Number(colony.saleUnitSize) : 1}</div>
            </div>
    ...
    html += `
      ...
      <div class="iso-section-head">
        ...
        ${help("This shows packaged inventory created from your colonies. You can return packaged items to restore the original colony quantity.")}
      </div>
    `;
    ...
    if (!packagedTypes.length) {
      ...
      packagedTypes.forEach(type => {
        ...
        const totalUnits = items.reduce((sum, x) => sum + (Number(x.quantity) || 0), 0);
        html += `
          <div class="iso-card">
            <div class="iso-card-head">
              <div>
              ...
              <span class="iso-badge">${totalUnits}</span>
            </div>
            <div class="iso-history-list">
              ${items.map(item => `
                <div class="iso-history-item">
                  ...
                  <div class="iso-history-text">
                    <strong>${esc(item.sourceColonyName)}</strong> — ${item.quantity} × ${esc(item.saleUnitLabel)}
                  </div>
                  <div class="iso-actions" style="margin-top:8px;">
    ...
    html += `
      ...
      <div class="iso-section-head">
        ...
        ${help("Start empty and add your own custom packaging supplies. When a colony is linked to a supply, prep subtracts one supply item per packaged unit.")}
      </div>
      <div class="iso-toolbar">
  ...
  async function prepFromColony(index) {
    ...
    if (colony.inventoryMode === "count") {
      amountToSubtract = qty * Math.max(1, toInt(colony.saleUnitSize, 1));
    }
    ...
    if (supply) {
      supply.quantity -= qty;
      addHistory(colony, "Packaging used", `Used ${qty} × ${supply.name}.`);
    }

    state.salePrep.packagedItems.push({
      id: uid(),
  ...
  function renderPriceSheetPreview() {
    ...
    const orderedKeys = [...new Set([...preset.priceSections, ...Object.keys(sections)])]
  ...
  function renderSettings() {
    ...
    app(`
      <div class="iso-section-head">
        ...
        ${help("Manage your backup, custom tags, type thresholds, and PWA install instructions here.")}
      </div>
      ...
      <div class="iso-section-head">
        ...
        ${help("Built-in tags are Breeding, For Sale, and Experimental. Add your own here and they will appear on colony forms and colony filters.")}
      </div>
      ...
      <div class="iso-section-head">
        ...
        ${help("Defaults are 3 days green and 10 days yellow. Set custom thresholds by type and by task to better match your husbandry style.")}
      </div>
  ...
  function renderThresholdEditor(typeName, thresholds) {
    ...
    return `
      ...
      <div class="iso-grid">
        ${taskRow("misting", "Misting")}
        ${taskRow("feeding", "Feeding")}
        ${taskRow("substrate", "Substrate Check")}
        ${taskRow("botanicals", "Botanicals Check")}
      </div>
      <div class="iso-actions">
